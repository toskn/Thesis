...
function compile_partition() {
    rm -f $2.$5.$3$4
    # запись собранных воедино скелетов конлла в общий файл
...
# это не нужно, все процесснуто, конлл собран
#compile_language english
#compile_language chinese
#compile_language arabic

# после этого скрипт запускает минимайз на файлы делая из коннла джсонлайнс
vocab_file=cased_config_vocab/vocab.txt
python minimize.py $vocab_file $data_dir $data_dir false


# получается суть скрипта в загрузке модели незатюненой на онтоноутс 
# и преобразовании файлов из конлл в джсонлайнс